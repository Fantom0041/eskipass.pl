...
ENV HTTP_PROXY=http://172.16.2.254:3128
ENV HTTPS_PROXY=http://172.16.2.254:3128
ENV NO_PROXY=localhost,127.0.0.1

# Configure proxy and DNS
RUN echo 'Acquire::http::Proxy "http://172.16.2.254:3128";' > /etc/apt/apt.conf.d/01proxy && \
...
# Configure PEAR and PECL with proxy
RUN pear config-set http_proxy http://172.16.2.254:3128 || true && \
    pecl channel-update pecl.php.net || true

# Download and install MongoDB extension manually
RUN curl -x http://172.16.2.254:3128 -L https://pecl.php.net/get/mongodb-1.15.0.tgz -o mongodb.tgz && \
