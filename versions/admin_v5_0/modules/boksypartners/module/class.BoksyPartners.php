...
class Boksy extends Main
{
    ...
    public function __construct()
    {
        ...
        if ($this->en->IsLoggedIn() == true) {
            //aktywna zakładka
            $this->tpl->assign("activetab", "boksypartners");
    ...
    private function DisplayList()
    {
        if ($this->en->IsLoggedIn() == true) {
            ...
            $dane = $this->en->select("SELECT * FROM `" . TABLE_PREFIX . "podstrony_boksy` WHERE `page`= 'partners' ORDER BY `id` ASC");
    ...
    {
        if (!file_exists('data/boksypartnerzy')) {
            mkdir('data/boksypartnerzy');
        }
        if (!file_exists('data/boksypartnerzy/' . $id)) mkdir('data/boksypartnerzy/' . $id);
        $ext = $this->add->FileExt($plik['name']);
        $nazwa = md5($plik['name'] . time()) . "." . $ext;
        move_uploaded_file($plik['tmp_name'], 'data/boksypartnerzy/' . $id . '/' . $nazwa);
        return 'data/boksypartnerzy/' . $id . '/' . $nazwa;
    }
