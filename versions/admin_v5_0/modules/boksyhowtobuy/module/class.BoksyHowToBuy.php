...
class Boksy extends Main
{
    ...
    public function __construct()
    {
        ...
        if ($this->en->IsLoggedIn() == true) {
            //aktywna zakładka
            $this->tpl->assign("activetab", "boksyhowtobuy");
    ...
    private function DisplayList()
    {
        if ($this->en->IsLoggedIn() == true) {
            ...
            $dane = $this->en->select("SELECT * FROM `" . TABLE_PREFIX . "podstrony_boksy` WHERE `page`= 'how_to_buy' ORDER BY `id` ASC");
    ...
    {
        if (!file_exists('data/boksyhowtobuy')) {
            mkdir('data/boksyhowtobuy');
        }
        if (!file_exists('data/boksyhowtobuy/' . $id)) {
            mkdir('data/boksyhowtobuy/' . $id);
        }
        $ext = $this->add->FileExt($plik['name']);
        $nazwa = md5($plik['name'] . time()) . "." . $ext;
        move_uploaded_file($plik['tmp_name'], 'data/boksyhowtobuy/' . $id . '/' . $nazwa);
        return 'data/boksyhowtobuy/' . $id . '/' . $nazwa;
    }
