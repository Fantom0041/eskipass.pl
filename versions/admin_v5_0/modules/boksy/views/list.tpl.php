    ...
    <div class="page-content">
        ...
        <div class="panel">
            <div class="panel-body">
                ...
                <table id="boksy-table" class="table table-hover dataTable table-striped width-full"
                    ...
                    <tr>
                        ...
                        <th>Kolejność</th>
                        <th>Typ</th>
                        <th class="toright"><a href="{{$siteUrl}}boksy/edit" class="btn btn-success btn-sm">utwórz</a>
                        </th>
                    </tr>
                    </thead>
                    <tbody>
                    ...
                    <tr>
                        <td>{{$dane[id].id}}</td>
                        <td>{{$dane[id].nazwa}}</td>
                        <td>{{if $dane[id].type == "osrodek"}}{{$dane[id].datastart_iso}}{{else}}<span
                                    style="font-size: 20px">&infin;</span>{{/if}}
                        </td>
                        <td>{{if $dane[id].type == "osrodek"}}{{$dane[id].datastop_iso}}{{else}}<span
                                    style="font-size: 20px">&infin;</span>{{/if}}
                        </td>
                        ...
                        <td>{{$dane[id].type}}</td>
                        <td class="toright">
                            {{if $dane[id].type == 'osrodek'}}
                            <button type="button" data-href="{{$siteUrl}}boksy?id={{$dane[id].id}}"
                                    data-name="{{$dane[id].nazwa}}" data-id="{{$dane[id].id}}"
                                    class="btn btn-danger btn-sm remove-boks">usuń
                            </button>
                            {{/if}}
                            <a href="{{$siteUrl}}boksy/edit?id={{$dane[id].id}}" class="btn btn-info btn-sm">edytuj</a>
                        </td>
                    </tr>
                    {{/section}}
                    </tbody>
...
<script>
    (function (document, window, $) {
        ...
        $(document).ready(function ($) {
            ...
            $(body).on('click', '.remove-boks', function () {
                console.log('heh');
